...
router.post('/', async (req:any,res:any) => {
    ...
    await client.connect(async (err:any, db:any) => {
        ...
        const myquery = {id: uuidv4(), title: req.body.title, description: req.body.description, date: req.body.date}
        await myDb.collection('events').insertOne(myquery).then((document:any) => {
            ...
            db.close()
            return res.status(200).send({success_message: 'Successfully added an event'})
        })
    }) 
})
